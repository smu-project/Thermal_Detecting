...
class ToQTensor:
    def __call__(self, img, target):
        return F.to_tensor(img).mul(255.), torch.tensor(target)
...
class Resize:
    def __init__(self, size, interpolation=Image.LANCZOS, use_cv2=False):
        self.size = _pair(size)
         
        # (h, w) -> (w, h)
        self.size = tuple([self.size[1], self.size[0]])
    ...
    def __call__(self, img, target):
        ...
        if self.use_cv2:
            return cv2.resize(img[...,(2, 1, 0)], size), target
        else:
            return img.resize(size, self.interpolation), target
...
class CenterCrop:
    ...
    def __call__(self, img, target):
        return F.center_crop(img, self.target_size), target
...
class ColorSpaceConvert:
    def __init__(self, method='RGBToIYU2'):
        def rgb_to_iyu2(act):
            c, h, w = act.shape
            iyu2 = torch.zeros_like(act)
            assert c == 3
            ...
            iyu2[0], iyu2[1], iyu2[2] = u, y, v

            return iyu2.round().clamp(0, 255)

        def iyu2_to_rgb(act):
            rgb = torch.zeros_like(act)
            c, h, w = act.shape
            assert c == 3

            u, y, v = act[0], act[1], act[2]
            r = y + 1.5390625 * v - 197
            g = y - 0.1796875 * u - 0.4609375 * v  + 82
            b = y + 1.8125 * u - 232
            r, g, b = torch.round(r), torch.round(g), torch.round(b)
            rgb[0], rgb[1], rgb[2] = r, g, b

            return rgb.round().clamp(0, 255)

        def rgb_to_argb(act, val=127):
            c, h, w = act.shape
            assert c == 3

            alpha = torch.zeros(1, h, w).add(val)
            rgba = torch.cat((act, alpha), 0)

            return rgba

        if method == 'RGBToIYU2':
            self.converter = rgb_to_iyu2
        elif method == 'IYU2ToRGB':
            self.converter = iyu2_to_rgb
        elif method == 'RGBToARGB':
            self.converter = rgb_to_argb
        else:
            raise Exception(f"Not supported color-format {method}")

    def __call__(self, img, target):
        return self.converter(img), target


class TensorFromImage:
...
class MakeNPUInput:
    ...
    def __call__(self, x, target):
        return x - 128.0, target

class QuantFriendlyNormalize:
    def __init__(self, scale):
        self.mean = [0.5, 0.5, 0.5]
        self.std = [scale/256., scale/256., scale/256.]
...
class ConvertGray2RGB:
    def __call__(self, img, target, use_cv2=False):
        if use_cv2:
...
class ExpandChannel:
    ...
    def __call__(self, img, target):
        ...
        m = (self.target_channel // channel) + 1

        expand_img = img.repeat(m, 1, 1)
        expand_img = expand_img[:self.target_channel, :, :]

        return expand_img, target
